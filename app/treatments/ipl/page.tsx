...
export const metadata: Metadata = {
  ...
  description: "Learn how IPL works, which skin concerns it may address, treatment areas, consultation pricing and what to expect.",
};
...
const areas = [
  ["Full face", "Pigmentation, visible redness and overall tone"],
  ["Face + neck", "A broader treatment area for a more even transition"],
  ["Décolletage", "Sun-related discolouration and visible redness"],
  ["Hands", "Visible sun spots and uneven-looking tone"],
  ["Spot treatment", "A small, practitioner-approved target area"],
] as const;
...
export default function IplTreatmentPage() {
  return <main><Header />
    ...
    <section className="ipl-pricing" id="pricing"><div className="section-heading"><div><p className="kicker">03 · Treatment areas</p><h2>Area-based<br /><i>pricing.</i></h2></div><p>IPL pricing is confirmed after your consultation because the treatment area, concern and required settings vary. All amounts will be quoted in AUD before treatment.</p></div><div className="ipl-area-table" role="table" aria-label="IPL treatment areas"><div className="ipl-area-row area-head" role="row"><span>Area</span><span>Common focus</span><span>Price</span></div>{areas.map(([area, focus]) => <div className="ipl-area-row" role="row" key={area}><strong>{area}</strong><span>{focus}</span><span>Consultation pricing</span></div>)}</div><p className="price-note">A patch test may be recommended. Your practitioner will confirm suitability, expected sessions, preparation, aftercare and total price before proceeding.</p></section>

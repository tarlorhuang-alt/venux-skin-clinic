...
export default async function ClientsAdmin({searchParams}:{searchParams:Promise<{saved?:string;error?:string;imported?:string;processed?:string;duplicates?:string;q?:string}>}){
  ...
  return <AdminShell active="Clients & membership">
    ...
    <form action={importClients} className="client-import-form"><div><strong>Import or export customer records</strong><span>CSV columns: Group, Name, DOB, Mobile, Email, Address. Existing clients are matched by Email or by Name + Mobile.</span><a className="client-export-link" href="/admin/clients/export">Export all clients CSV</a></div><input type="file" name="clientsFile" accept=".csv,text/csv" required/><button type="submit">Import CSV</button></form>

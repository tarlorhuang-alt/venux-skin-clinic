...
export async function GET(){
  ...
  const headers=["Client ID","Name","Mobile","Email","DOB","Address","Group","Source","Membership","Card Balance","Membership Paid","Appointments","Last Visit"];
  const body=[headers.map(csv).join(","),...rows.map(row=>[row.id,row.full_name,row.mobile,row.email,row.dob,row.address,row.customer_group,row.lead_source,row.membership_status,row.membership_balance,row.membership_amount_paid,row.appointment_count,row.last_visit].map(csv).join(","))].join("\r\n");

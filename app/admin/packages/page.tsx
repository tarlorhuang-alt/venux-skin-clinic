...
export default async function PackagesPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
  ...
  return <AdminShell active="Packages"><header className="clinic-admin-head"><div><p>Treatment plans & sessions</p><h1>Client packages</h1></div><span>Sessions deduct when appointments are completed</span></header>
    ...
    <section className="package-top">
      {role==="owner"?<form action={createPackageAction} className="ops-card package-form"><h2>Create or update a package</h2><label>Package name<input name="name" required placeholder="e.g. Acne recovery course"/></label><div className="package-form-row"><label>Package price (AUD)<input name="price" type="number" min="0" step="0.01" required/></label><label>Valid for days<input name="validityDays" type="number" min="1" defaultValue="365" required/></label></div><p>Select each treatment and enter its included sessions. Assigning this package creates all selected treatment balances automatically.</p>{[1,2,3,4,5].map(index=><div className="package-item-input" key={index}><select name={`serviceId${index}`} defaultValue=""><option value="">Treatment {index}</option>{activeServices.map(service=><option value={String(service.id)} key={String(service.id)}>{String(service.category)} · {String(service.service_name)}</option>)}</select><input name={`sessions${index}`} type="number" min="1" placeholder="Sessions"/></div>)}<button>Save package & treatment sessions</button></form>:<article className="ops-card package-form"><h2>Package templates</h2><p>Package templates are managed by the owner. Staff can assign an existing package to a client below.</p></article>}

...
export function AppointmentClientPicker({initialClient}:{initialClient?:ClientSuggestion}){
  ...
  return <div className="appointment-client-picker">
    ...
    <label className="client-name-field">Client name<div className="client-combobox"><input name="name" required value={name} onFocus={()=>setOpen(true)} onChange={event=>{setName(event.target.value);if(clientId){setClientId(0);setMobile("");setEmail("");setClinicLocation("");}}} autoComplete="off" role="combobox" aria-expanded={open} aria-controls="appointment-client-options" placeholder="Type to find an existing client"/>{clinicLocation?<em className={`selected-location clinic-location-badge ${clinicLocation==="City"?"city":"ryde"}`}>{clinicLocation}</em>:null}{open?<div className="client-options" id="appointment-client-options" role="listbox">{results.length?results.map(client=><button type="button" role="option" key={client.id} onMouseDown={event=>event.preventDefault()} onClick={()=>choose(client)}><strong>{client.name}<em className={`clinic-location-badge ${client.clinicLocation==="City"?"city":"ryde"}`}>{client.clinicLocation}</em></strong><span>{client.mobile}{client.email?` · ${client.email}`:""}</span><small>{client.visitCount} appointments · {client.membershipStatus||"non-member"}</small></button>):name.trim()?<p>No matching client — a new profile will be created.</p>:<p>Start typing to search recent clients.</p>}</div>:null}</div></label>

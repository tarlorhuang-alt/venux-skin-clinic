...
export function AppointmentClientPicker({initialClient}:{initialClient?:ClientSuggestion}){
  ...
  useEffect(()=>{if(skipNext.current){skipNext.current=false;return;}const controller=new AbortController(),timer=window.setTimeout(async()=>{try{const response=await fetch(`/api/admin/clients/search?q=${encodeURIComponent(name.trim())}`,{signal:controller.signal,cache:"no-store"});if(!response.ok)return;const data=await response.json() as {clients:ClientSuggestion[]};setResults(data.clients);setOpen(true);}catch(error){if((error as Error).name!=="AbortError")setResults([]);}},250);return()=>{window.clearTimeout(timer);controller.abort();};},[name]);

...
export function TreatmentPicker({services}:{services:TreatmentOption[]}){
  ...
  return <div className="treatment-picker">
    ...
    <label>Treatment<div className="treatment-combobox"><input name="treatment" required value={query} onFocus={()=>{if(query.trim().length>=2)setOpen(true);}} onChange={event=>{setQuery(event.target.value);setServiceId(0);setOpen(event.target.value.trim().length>=2);}} autoComplete="off" role="combobox" aria-expanded={open} aria-controls="appointment-treatment-options" placeholder="Search or type a new treatment"/>{open?<div className="treatment-options" id="appointment-treatment-options" role="listbox">{matches.length?matches.map(service=><button type="button" role="option" key={service.id} onMouseDown={event=>event.preventDefault()} onClick={()=>choose(service)}><strong>{service.name}</strong><span>{service.category} · ${service.regularPrice.toLocaleString("en-AU")}</span></button>):<button type="button" className="custom-treatment-option" onMouseDown={event=>event.preventDefault()} onClick={()=>setOpen(false)}><strong>Use “{query.trim()}” as a new treatment</strong><span>One-off booking · enter the agreed price below</span></button>}</div>:null}</div></label>
